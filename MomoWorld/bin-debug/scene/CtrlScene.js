...
var CtrlScene = (function (_super) {
    ...
    function CtrlScene() {
        ...
        this.bagBtnName = "Equip"; //背包选择卡的下标
    ...
    //显示顶部层
    p.showTop = function (ctrl) {
        if (ctrl === void 0) { ctrl = "show"; }
        if (ctrl == "show") {
            this.isTop = true;
    ...
    //显示操作层
    p.showCtrl = function (type, index) {
    ...
    p.ctrlData = function (type, detailType) {
        ...
        if (type == "show") {
            ...
            dataAddBackground.addEventListener(egret.TouchEvent.TOUCH_TAP, function (e) {
                if (e.localX > 75 && e.localX < 100 && e.localY > 0 && e.localY < 25)
            ...
            for (var i = 0; i < this.dataAddData.length; i++) {
                ...
                this.dataAddIcon[i].addEventListener(egret.TouchEvent.TOUCH_END, function (e) {
    ...
    p.ctrlBag = function (type, sure) {
        ...
        if (type == "show") {
            ...
            this.bag_btnEquipment.addEventListener(egret.TouchEvent.TOUCH_TAP, function (e) {
                e.stopImmediatePropagation();
            ...
            this.bag_btnPiece.addEventListener(egret.TouchEvent.TOUCH_TAP, function (e) {
                e.stopImmediatePropagation();
                if (this.bagBtnName == "Piece")
                    return;
                this.bagBtnName = "Piece";
                this.ctrlBag("changeBtn");
            }, this);
            this.bag_btnDrup.addEventListener(egret.TouchEvent.TOUCH_TAP, function (e) {
                e.stopImmediatePropagation();
            ...
            for (var i = 0; i < 5; i++) {
                ...
                for (var a = 0; a < 7; a++) {
                    var index = i * 7 + a;
                    if (index >= GameData["bag_" + this.bagBtnName].length)
                        break;
            ...
            if (this.bagDetailContainer && !sure)
                return;
